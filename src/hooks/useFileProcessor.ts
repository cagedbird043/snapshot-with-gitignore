...
export const useFileProcessor = () => {
  ...
  useEffect(() => {
    ...
    worker.onmessage = (event: MessageEvent<FilterWorkerOutboundMessage>) => {
      ...
      if (message.type === 'FILTER_RESULT') {
        ...
        dispatch({ type: 'FILTERING_FAILED', payload: { status: message.payload.message } });
      }
      terminateWorker(filterWorkerRef);
    };
  ...
  const generateSnapshot = useCallback(() => {
    if (!state.filteredFiles.length) {
      dispatch({ type: 'SET_STATUS', payload: { status: 'No files to process after filtering.' } });
      return;
    }
    ...
    worker.onerror = error => {
      ...
      dispatch({
        ...
        payload: { status: `An error occurred during snapshot generation: ${error.message}` },
      });
      terminateWorker(snapshotWorkerRef);
    };
